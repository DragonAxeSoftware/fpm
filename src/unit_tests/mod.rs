...
fn test_full_install_workflow() -> Result<()> {
    ...
    assert!(
        martha_bundle_dir.exists(),
        "Martha's bundle should be installed"
    );
    assert!(
        icons_bundle_dir.exists(),
        "Icons bundle should be installed"
    );
    ...
    assert!(
        martha_manifest_path.exists(),
        "Bundle should have its own manifest"
    );
...
fn test_nested_bundles_workflow() -> Result<()> {
    ...
    assert!(
        ...
        "Nested bundle should be installed"
    );
...
fn test_source_bundle_status() -> Result<()> {
    ...
    let loaded_manifest = load_manifest(&manifest_path)?;
    assert!(
        loaded_manifest.is_source_bundle(),
        "Should be a source bundle"
    );
    ...
    let has_source = statuses
        .iter()
        .any(|(_, status)| *status == BundleStatus::Source);
